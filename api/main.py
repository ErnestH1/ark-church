...
from app.videos import router as videos_router
from app.podcasts import router as podcasts_router
from app.config import DROPBOX_ACCESS_TOKEN
...
@app.get("/dashboard/", response_class=HTMLResponse)
async def dashboard(request: Request):
    # Check if the access token is present in the config
    access_token = DROPBOX_ACCESS_TOKEN

    if not access_token:
        # If access token is missing, handle it accordingly (e.g., redirect to login page)
        return HTMLResponse("Access token not found. Please log in.")
